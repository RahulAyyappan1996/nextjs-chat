...
export function Stock({ props: { symbol, price, delta } }: { props: Stock }) {
  ...
  const { width = 0 } = useResizeObserver<HTMLDivElement>({
    ref: chartRef as React.RefObject<HTMLDivElement | null> as any,
    box: 'border-box'
  })
      ...
      >
        {priceAtTime.x > 0 ? (
          <div
            ...
            ref={node => {
              if (node) {
                node.style.left = `${priceAtTime.x - 124 / 2}px`
                node.style.top = '30px'
              }
            }}
          >
        ...
        {startHighlight ? (
          <div
            ...
            ref={node => {
              if (node) {
                node.style.left = `${startHighlight}px`
                node.style.width = `${endHighlight ? endHighlight - startHighlight : priceAtTime.x - startHighlight}px`
